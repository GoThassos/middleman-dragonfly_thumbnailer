  ...
  module DragonflyThumbnailer
    class Extension < Middleman::Extension
      ...
      def thumb(path, geometry)
        ...
        image.convert!('-quality 82 -strip -interlace none -filter Triangle -define filter:support=2 -unsharp 0.25x0.08+8.3+0.045 -colorspace sRGB -define jpeg:fancy-upsampling=off')
      ...
      helpers do
        def thumb_url(path, geometry)
          extensions[:dragonfly_thumbnailer].image_url(path, geometry)
        end
        def thumb_tag(path, geometry, options = {})
          url = extensions[:dragonfly_thumbnailer].image_url(path, geometry)
